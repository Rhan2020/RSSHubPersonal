...
import { highSalaryKeywords, remoteKeywords, painPointKeywords, excludeKeywords } from '../config/keywords';
...
function isHighValueJob(item: JobItem): boolean {
    ...
    if (hasExcludeKeywords(title)) {return false;}

    // 排除明显的后端岗位
    const backendKeywords = ['python', 'java', 'golang', 'rust', 'c++', 'ruby', 'php', 'backend', 'back-end', 'devops', 'data', 'ml', 'machine learning', 'ai engineer', 'infrastructure'];
    const isBackend = backendKeywords.some((k) => title.includes(k));
    if (isBackend && !title.includes('fullstack') && !title.includes('full-stack')) {return false;}

    // 检查是否包含前端相关关键词
    const frontendSpecificKeywords = ['frontend', 'front-end', 'front end', 'react', 'vue', 'angular', 'javascript', 'typescript', 'ui', 'ux', 'web developer', 'web engineer'];
    const isFrontend = frontendSpecificKeywords.some((k) => text.includes(k.toLowerCase()));
    ...
    const hasGoodSalary = salary >= 60000; // 年薪6万美元起（月薪5k）
    ...
    // 检查是否包含远程关键词
    const hasRemote = isRemotePlatform || remoteKeywords.some((k) => text.includes(k.toLowerCase()));
    ...
    // 评分系统（满足条件越多，分数越高）
    let score = 0;
    if (isFrontend) {score += 3;} // 前端相关（加权更高）
    if (isFullstack) {score += 2;} // 全栈相关
    ...
    // 特殊情况：某些组合直接通过
    if (isFrontend && hasRemote) {return true;} // 前端+远程
    if (isFrontend && hasGoodSalary) {return true;} // 前端+高薪
    if (isFullstack && hasRemote) {return true;} // 全栈+远程
    if (isFullstack && hasGoodSalary) {return true;} // 全栈+高薪
    if (isRemotePlatform && isFrontend) {return true;}
    if (fromHighSalaryPlatform && (isFrontend || isFullstack)) {return true;}

    // 特定平台的前端相关职位通过
    if (item.sourceName.includes('RemoteOK') && (isFrontend || isFullstack)) {return true;}
    if (item.sourceName.includes('V2EX') && (isFrontend || isFullstack)) {return true;}
    if (item.sourceName.includes('Working Nomads') && (isFrontend || isFullstack)) {return true;}

    // 分数>=3就认为是高价值工作（提高门槛，确保质量）
    return score >= 3;
}
...
function isFromHighSalaryPlatform(sourceName: string): boolean {
    const highSalaryPlatforms = ['AngelList', 'Toptal', 'Turing', 'Arc.dev', 'X-Team', 'Gun.io', 'Gigster', 'Hired', 'YCombinator', 'CryptoJobs', 'Web3 Jobs', 'DeFi Jobs'];

    return highSalaryPlatforms.some((platform) => sourceName.toLowerCase().includes(platform.toLowerCase()));
}

// 商机平台
function isIdeaSourcePlatform(sourceName: string): boolean {
    const ideaPlatforms = ['Reddit r/SomebodyMakeThis', 'Reddit r/AppIdeas', 'Reddit r/Startup_Ideas', 'Reddit r/Business_Ideas', 'Product Hunt', 'Indie Hackers', 'V2EX 奇思妙想'];

    return ideaPlatforms.some((platform) => sourceName.toLowerCase().includes(platform.toLowerCase()));
}
...
export function extractSalary(text: string): number {
    ...
    for (const pattern of patterns) {
        ...
        if (match) {
            ...
            // 处理范围（取平均值）
            if (match[2]) {
                const amount2 = Number.parseInt(match[2]);
                if (amount2 > amount) {
...
// 高级筛选函数
export function advancedFilter(items: JobItem[], options: AdvancedFilterOptions): JobItem[] {
    return items.filter((item) => {
        ...
        // 地区筛选
        if (options.regions && options.regions.length > 0 && !options.regions.includes(item.region)) {return false;}

        // 来源筛选
        if (options.sources && options.sources.length > 0 && !options.sources.some((s) => item.sourceName.includes(s))) {return false;}
