...
export async function fetchSource(source: DataSource): Promise<JobItem[]> {
    try {
        ...
        // 检查是否是社交媒体数据源
        const socialPlatforms = ['LinkedIn', 'Twitter', 'Facebook', 'Instagram', 'Discord', 
                                'Telegram', 'Slack', 'Medium', 'TikTok', 'Threads', 
                                'Mastodon', 'YouTube', 'Pinterest', 'Quora'];
        const isSocialMedia = socialPlatforms.some(platform => 
            source.category?.toLowerCase() === platform.toLowerCase() || 
            source.name.includes(platform)
        );
        ...
        if (isSocialMedia) {
            ...
            switch (source.dataType) {
                ...
                case 'html':
                    ...
                    break;
                default:
                    console.warn(`Unsupported data type: ${source.dataType} for ${source.name}`);
            }
        }
...
async function fetchGenericJSON(source: DataSource): Promise<JobItem[]> {
    try {
        ...
        return items.slice(0, 50).map((item: any) => {
            ...
            // 处理描述
            let description = item.description || item.summary || item.selftext || '';
        ...
        console.error(`Failed to parse JSON from ${source.name}:`, error);
        return [];
    }
}
...
export async function fetchMultipleSources(sources: DataSource[]): Promise<JobItem[]> {
    console.log(`开始抓取 ${sources.length} 个数据源...`);
    ...
    for (let i = 0; i < sources.length; i += batchSize) {
        ...
        console.log(`已完成 ${Math.min(i + batchSize, sources.length)}/${sources.length} 个数据源`);
    }
    
    console.log(`抓取完成，共获取 ${results.length} 条数据`);
    return results;
}
